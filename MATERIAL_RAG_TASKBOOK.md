...
当前状态为 `UAT_MACHINE_GATE_PASSED / LOCAL_SYNTHETIC_BROWSER_UAT_PASSED / HUMAN_UAT_NOT_READY / HUMAN_UAT_SIGNOFF_PENDING / LIVE_RETRIEVAL_UAT_BLOCKED_BY_KEY_ROTATION / NOT_PRODUCTION`。当前分支精确为 `codex/material-rag-scanner-protocol`，HEAD `a72fdb186de2ab53f6c8d72983f1b24fc99dac1e`。不是 `UAT_PASSED`，不是 `RELEASE_VERIFIED`，未部署、未生产。`ARK_KEY_ROTATION_REQUIRED`。本窗口 live 周期2 取得 `LOCAL_MATERIAL_RAG_UAT_LIVE_BROWSER_OK`。22:45 `UAT_TENANT_OPTION_MISSING` 与 20:24 清空假绿、旧 `REQUEST_NOT_SENT` / `UAT_PHASE_MISSING` 均为历史，不得合并计数。未写 `HUMAN_UAT_READY`。
