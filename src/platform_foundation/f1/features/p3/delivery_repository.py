"""PostgreSQL truth for body-free controlled-ingestion delivery."""
...
class MaterialIngestionDelivery:
    id: uuid.UUID
...
@dataclass(frozen=True, slots=True)
class MaterialIngestionDeliveryClaim:
    id: uuid.UUID
    enterprise_id: uuid.UUID
    document_version_id: uuid.UUID
    actor_sub: str
...
def delivery_enabled() -> bool:
    return (
        os.environ.get(DELIVERY_FLAG) == "1"
        and os.environ.get(_ENGINEERING_FLAG) == "1"
    )
...
def delivery_id_for(
    ...
    if not all(
        ...
        for value in (enterprise_id, document_version_id)
    ):
        raise ValueError("MATERIAL_INGESTION_DELIVERY_IDENTITY_INVALID")
    identity = (
        f"material-ingestion:resume:{enterprise_id}:{document_version_id}"
    )
...
def _actor_sub(value: str) -> str:
    if (
        ...
        raise ValueError("MATERIAL_INGESTION_ACTOR_INVALID")
    return value


def _delivery(row) -> MaterialIngestionDelivery:
    return MaterialIngestionDelivery(
        id=row["id"],
...
async def register_delivery_in_session(
    ...
    *,
...
) -> MaterialIngestionDelivery:
    """Write the stable delivery inside the caller's current transaction."""
    ...
    row = (
        await session.execute(
            text(
                ...
                "FROM f1.register_material_ingestion_delivery("
    ...
    if row is None or row["id"] != stable_id:
        raise RuntimeError("MATERIAL_INGESTION_DELIVERY_IDENTITY_CONFLICT")
...
async def claim_due_deliveries(
    *, limit: int = 100, lease_seconds: int = 1800
) -> tuple[MaterialIngestionDeliveryClaim, ...]:
    async with session_scope(role="f1_worker") as session:
        rows = (
            await session.execute(
                text(
                    ...
                    "FROM f1.claim_material_ingestion_deliveries("
                    ":limit,:lease_seconds)"
                ),
        ...
        await session.commit()
    return tuple(
        MaterialIngestionDeliveryClaim(
            id=row["delivery_id"],
            enterprise_id=row["enterprise_id"],
            document_version_id=row["document_version_id"],
            actor_sub=str(row["actor_sub"]),
...
async def read_delivery_claim(
    ...
    *,
    ...
    if row is None:
        return None
    return MaterialIngestionDeliveryClaim(
        id=row["delivery_id"],
        enterprise_id=row["enterprise_id"],
        document_version_id=row["document_version_id"],
        actor_sub=str(row["actor_sub"]),
...
async def purge_expired_ocr_checkpoints(*, limit: int = 256) -> int:
    async with session_scope(role="f1_worker") as session:
        deleted = int(
            (
                await session.execute(
                    text(
                        "SELECT f1.purge_expired_material_ocr_checkpoints(:limit)"
                    ),
                    {"limit": limit},
                )
            ).scalar_one()
