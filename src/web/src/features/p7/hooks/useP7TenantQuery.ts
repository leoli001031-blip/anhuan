...
import { userFacingLocalRehearsalError } from "../localRehearsalApi";
...
export function useP7TenantQuery<T>(initialData: T, loader: QueryLoader<T>) {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      if (!controller.signal.aborted) setError(userFacingLocalRehearsalError(reason));
    } finally {
  ...
  const runMutation = useCallback(async <R,>(operation: MutationLoader<R>): Promise<R> => {
    activeMutation.current?.abort();
    const controller = new AbortController();
    activeMutation.current = controller;
    try { return await operation(getAccessToken(), controller.signal); }
    finally { if (activeMutation.current === controller) activeMutation.current = null; }
  }, [getAccessToken]);

  return { data, setData, loading, error, setError, reload, runMutation, tenantEpoch };
}
