...
import { userFacingAutomatedQualityError } from "../automatedQualityApi";
...
export function useP6TenantQuery<T>(initialData: T, loader: QueryLoader<T>) {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      if (!controller.signal.aborted) setError(userFacingAutomatedQualityError(reason));
    } finally {
