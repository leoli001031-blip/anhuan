#!/usr/bin/env python3
"""Generate deterministic synthetic F0-E PDF/JPEG envelopes for local probes."""
...
def _minimal_pdf() -> bytes:
    stream = b"BT /F1 36 Tf 72 600 Td (F0E CANARY 123) Tj ET\n"
    objects = [
...
def _jpeg(with_text: bool = True) -> bytes:
    ...
    if with_text:
        ...
        draw.text((36, 78), "JPEG CANARY 456", fill="black", font=font)
    output = io.BytesIO()
    image.save(output, format="JPEG", quality=90, optimize=False, progressive=False)
    return output.getvalue()


def _envelope(probe_type: str, tamper_hash: bool) -> bytes:
    document_type = "PDF" if probe_type == "PDF" else "JPEG"
    if probe_type == "PDF":
        source = _minimal_pdf()
    else:
        source = _jpeg(with_text=probe_type == "JPEG")
    ...
    header = {
        ...
        "source_unit_id": hashlib.sha256(
            f"f0e-synthetic-{probe_type.lower()}-page-1".encode("ascii")
        ).hexdigest(),
    }
...
def main() -> None:
    ...
    envelope = _envelope(args.document_type, args.tamper_hash)
