...
def upgrade() -> None:
    ...
    op.execute(
        ...
        BEGIN
          ...
          -- UNARCHIVE: transitioning FROM archived to not-archived.
          -- Only archive columns may change; mixing business-field updates
          -- into an unarchive is rejected so the guard stays airtight.
          IF OLD.archived_at IS NOT NULL THEN
            IF NEW.current_version_id IS DISTINCT FROM OLD.current_version_id
               OR NEW.current_version_no IS DISTINCT FROM OLD.current_version_no
               OR NEW.client_visible IS DISTINCT FROM OLD.client_visible THEN
              RAISE EXCEPTION 'ANALYSIS_REPORT_UNARCHIVE_MIXED';
            END IF;
            RETURN NEW;
          END IF;

          -- Report is NOT archived: business
...
def downgrade() -> None:
    # Restore the pre-f1_0025 guard verbatim before dropping the archive
    # columns: the upgraded function references them and would fail on any
    # later write once the columns are gone.
    op.execute(
        """
        CREATE OR REPLACE FUNCTION f1.guard_analysis_report_write()
        RETURNS trigger LANGUAGE plpgsql SECURITY INVOKER
