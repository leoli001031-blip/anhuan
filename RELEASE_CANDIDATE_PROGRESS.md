...
## 2026-09-02 `f1_0024` 云 OCR 远端部署 + 扫描件全链路 E2E 通过

- 部署编排：`docker-compose.analysis-report-cloud-ocr.yml`（no-ocr 之上叠层：sidecar 保持禁用、云 OCR env 注入 api/worker/dispatcher/ingestion-worker/report-worker、宿主 key 只读 bind mount，缺 key 插值即失败）+ `A_ECO_ANALYSIS_REPORT_OCR_MODE=cloud`（独立 workspace 身份、无 0600 key 文件拒绝启动）；`_write_env` 把 `A_ECO_CLOUD_OCR_*` 持久化进 compose env 文件（修 run:INTERPOLATE 失败）；`local_candidate`/seed/DEPLOYMENT 的 head 断言同步 0024。
- 服务器（腾讯轻量 `124.220.1.182`，ubuntu）：SSH 密码登录后为本机部署装 ed25519 公钥（加固项先行）；`c160c28+` 提交打包（git archive，SHA 双侧一致）→ 新 release 目录；GLM key 置于 `/home/ubuntu/aeco-test/secrets/glm_api_key`（0600，仓外）；旧 f1_0023 栈仅 `stop`（10 容器/16 卷/2 网保留为回滚点）；新栈 `anhuan-ar-demo-5508e0ac08b6` 启动：11/11 healthy、`f1_head=f1_0024`、双登录/工作流/ready 全绿，web 仅 loopback:60063。
- **远端扫描件 E2E 通过**：DCTDecode-only 扫描页（中文安环文本渲染 JPEG）上传 `202` → ClamAV clean → 分析 `OCR_APPLIED`、116 字符（GLM-5.3-Flash，anthropic 方言，服务器容器内真实外呼）→ 本地索引 canonical units → 自动报告草稿 v1（7 节、8 引用、含该扫描版本）；管道四阶段 ingestion/analysis/index/report 全 `ready`。回执：服务器 `/home/ubuntu/aeco-test/smoke-evidence/scanned-pdf-cloud-ocr-smoke-20260902.json`（0600）。
