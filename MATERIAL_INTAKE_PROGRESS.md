...
- 新增线性迁移 `f1_0012 → f1_0013`：持久化一张知识域表，旧文档一律无推断回填到服务公司域；新租户或新客户在首个授权上传事务中按唯一键惰性创建知识域。
...
- 该材料录入切片在正常验证收口时的源码与实库 head 均为 `f1_0014`，业务表目录为 35；`f1_0014` 把知识域继续带到底层原件与受控上传任务，历史 31/34 表备份恢复入口保留兼容。后续物理 RAG 的 `f1_0015 / 38` 仅由独立切片和专属栈使用。
...
- 代表性 Demo PDF 验证暴露了真实产品缺口：报告材料也曾获得政策草稿动作。新增 `f1_0011 → f1_0012`，不新增表，在文档和分析记录上保存上传预分类、机器建议及最终人工分类。
...
- 材料录入主链未执行黑客松 Demo 中的真实材料 PDF、批量浏览器流程、OCR、备份恢复实跑或候选准确率评估；仅一份确定性合成文本 PDF 在两个知识域各上传一次并通过。
- 扫描型 PDF 仍只标 `OCR_REQUIRED`，不执行 OCR；confidence 仍是未校准线索。
- 未部署，也未恢复 F1.1.1 发布验收。
- 新的人工分流与公司／客户知识归属尚未跑浏览器或代表性 Demo PDF 回归。
- `f1_0015` 物理 RAG 是后续独立切片。最近一次完整离线目标检查快照为 27/27；其后的 provider 诊断增量只做了 1 项定向检查。最新专属重放停在 `LOCAL_MATERIAL_RAG_PROVIDER_PROVISION_FAILED`，未进入索引、检索、重建或删除，也未取得 Ark embedding 聚合审计证据；详见 [MATERIAL_RAG_PROGRESS.md](./MATERIAL_RAG_PROGRESS.md) 与 [MATERIAL_RAG_BLOCKED.md](./MATERIAL_RAG_BLOCKED.md)。
